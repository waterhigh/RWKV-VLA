...
class VisualRWKV(pl.LightningModule):
    ...
    def get_global_condition(self, samples):
        device, label_dtype = samples["labels"].device, samples["labels"].dtype
        emb_dtype = samples["images"].dtype
        ...
        max_image_token_indice = self.get_max_image_token_indice(samples)
        self.img_start = max_image_token_indice
        self.img_end = max_image_token_indice + (image_features.shape[1] - 1)
        ...
        for idx, cur_input_ids in enumerate(samples["input_ids"]):
            ...
            sequence_lengths.append(cur_new_input_embeds.shape[0])

        max_len = max(x.shape[0] for x in new_input_embeds)
        batch_size = len(new_input_embeds)
        input_embeds_padded = torch.zeros((batch_size, max_len, self.args.n_embd), dtype=emb_dtype, device=device)
        for i, cur_embed in enumerate(new_input_embeds):
            cur_len = cur_embed.shape[0]
            input_embeds_padded[i, :cur_len] = cur_embed
            
        x = input_embeds_padded
        args = self.args
        if args.dropout > 0:
            x = self.rwkv.drop0(x)
        ...
        hidden_states = self.rwkv.ln_out(x) # [B, T, D] 
        last_token_indices = torch.tensor([length - 1 for length in sequence_lengths], device=device)
        global_condition = hidden_states[torch.arange(batch_size, device=device), last_token_indices]
        #[B, n_embd]
